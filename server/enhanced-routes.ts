...
enhancedRouter.patch('/admin/contractors/:id', isAuthenticated, requireRole(['admin']), async (req: Request, res: Response) => {
  try {
    ...
    // Get the contractor to access user data
    ...
    if (updateData.hourlyRate !== undefined) contractorUpdateData.hourlyRate = updateData.hourlyRate;
    ...
    // Update contractor
    const updatedContractor = await storage.updateContractor(id, contractorUpdateData);
...
enhancedRouter.delete('/admin/contractors/:id', isAuthenticated, requireRole(['admin']), async (req: Request, res: Response) => {
  try {
    ...
    await storage.updateContractor(id, { isActive: false });
    await storage.updateUser(contractor.userId, { role: 'archived' });

    res.json({ 
      message: 'Contractor archived successfully. Data will be permanently deleted after 60 days.'
    });
  } catch (error) {
