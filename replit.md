...
- **Bid Request Form UX Enhancement**: Streamlined bid request form interface by removing duplicate close button (kept only Cancel button), replaced problematic checkboxes with large, clickable service selection blocks featuring clear visual feedback, blue styling for selected services with checkmarks, and added instructional text "Select the service you require". Fixed CSS specificity issues in base Checkbox component affecting visibility across the application. Enhanced user experience with hover effects and improved readability.
- **Production Database Protection**: Implemented environment-based database initialization to prevent production data loss during deployments. Database seeding, commission data updates, and authentication fixes now only run in development mode. This ensures that live production data (contractors, salespeople, bid requests, etc.) added via the web interface remains intact during redeployments. Production deployments skip all database seeding operations to preserve real-world data integrity.
