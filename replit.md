...
- **Conversion Tracking Definition & Implementation (August 9, 2025)**: Implemented proper conversion tracking based on user-defined criteria: "Conversions = visit to site from salesperson attributed link + sending a bid request". Added automatic conversion tracking in bid request creation process - when a customer submits a bid request with salesperson attribution, the system now increments the salesperson's successfulConversions count via incrementSalespersonStats method. Updated Sales Dashboard to use database-tracked conversions and display "visit-to-bid rate" terminology. This provides accurate measurement of salesperson effectiveness in converting QR/NFC visits into actual bid requests.
